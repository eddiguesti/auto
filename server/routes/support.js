...
if (!isTelegramConfigured) {
  console.warn('[Support] Telegram not configured - escalation will be disabled')
}
...
async function sendToTelegram({ message, userEmail, userName, conversationHistory = [] }) {
  if (!isTelegramConfigured) {
    console.error('[Support] Cannot send to Telegram - not configured')
    return false
  }
...
_Sent from Easy Memoir Help Chat_`
    ...
    if (!data.ok) {
      console.error('[Support] Telegram API error:', data.description)
      return false
    }
    ...
    if (err.name === 'AbortError') {
      console.error('[Support] Telegram request timed out')
    } else {
      console.error('[Support] Failed to send to Telegram:', err.message)
    }
    return false
  }
}
