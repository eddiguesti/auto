...
export async function selectDailyPrompt(userId, userContext = {}) {
  try {
    ...
    if (candidates.rows.length === 0) {
      ...
      if (fallback.rows.length === 0) {
        ...
        const anyPrompt = await pool.query(
          `SELECT id, prompt_text, prompt_type, chapter_hint, question_hint, is_active, times_used, min_streak_days, tags, personality_tags, created_at FROM prompt_library WHERE is_active = true ORDER BY RANDOM() LIMIT 1`
        )
    ...
    if (chapterGap) {
      fullContext.selectionReason =
        chapterGap.answered === 0
...
export async function createDailyPrompt(userId) {
  ...
  try {
    ...
    const existing = await pool.query(
      `SELECT id, user_id, prompt_date, prompt_text, prompt_type, chapter_id, question_id, is_completed, completed_at, xp_awarded, created_at FROM daily_prompts WHERE user_id = $1 AND prompt_date = $2`,
      [userId, today]
    )
