...
async function getOrCreateGameState(userId) {
  ...
  let result = await pool.query(`SELECT * FROM user_game_state WHERE user_id = $1`, [userId])
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Get the prompt
    const prompt = await pool.query(`SELECT * FROM daily_prompts WHERE id = $1 AND user_id = $2`, [
      promptId,
      userId
    ])
