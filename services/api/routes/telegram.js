...
/**
 * Validate Telegram webhook request using secret token
 * Set the secret when configuring webhook:
...
function validateTelegramWebhook(req) {
  ...
  // Reject all requests if no secret configured
  if (!secret) {
    ...
    questions: [
      {
        ...
        prompt: 'The hospital, city, or country. Do you know any stories about the day you arrived?'
      },
      {
        id: 'first-memory',
        question: "What's your very first memory?",
        prompt: "Don't worry if it's fuzzy - even fragments are precious."
      },
      {
        id: 'childhood-home',
    ...
    questions: [
      {
        ...
        prompt: 'Who did you play with? Were you outside or inside?'
      },
      {
        id: 'summer-holidays',
        question: 'Describe a typical summer day during the holidays.',
        prompt: 'From when you woke up to when you went to bed.'
      },
      {
        id: 'best-friend',
        question: 'Who was your best friend?',
        prompt: 'What adventures did you have together?'
      }
    ]
  },
  {
    ...
    questions: [
      {
        ...
        prompt: 'What was she like? What do you remember most about her?'
      },
      {
        id: 'father',
        question: 'Tell me about your father.',
        prompt: 'What was he like? What do you remember most about him?'
      },
      {
        id: 'grandparents',
        question: 'What do you remember about your grandparents?',
        prompt: 'Did you know them? Where did they live?'
      }
    ]
  }
]
...
async function sendTelegramMessage(chatId, text, options = {}) {
  ...
  try {
    const response = await fetch(`${CONFIG.apiUrl}${CONFIG.botToken}/sendMessage`, {
      method: 'POST',
    ...
    if (!data.ok) {
      logger.error('API error', { error: data.description })
      return false
    }
    return data.result
  } catch (err) {
...
async function handleInterview(db, telegramUser, session, chatId) {
  if (!telegramUser.is_registered && !telegramUser.user_id) {
    await sendTelegramMessage(chatId, `Please complete registration first by typing /start`)
    return
  }
  ...
  if (!next) {
    await sendTelegramMessage(
      ...
      `🎉 *Congratulations!*\n\n` +
        ...
        (userId
          ? `They're already synced with your account.`
          : `Type /link to connect your account and access them online.`)
    )
...
router.post('/verify-link', requireDb, async (req, res) => {
  try {
    ...
    if (sessionResult.rows.length > 0) {
      const context =
        typeof sessionResult.rows[0].context === 'string'
      ...
      if (context.answers) {
        ...
        await db.query('UPDATE telegram_sessions SET context = $1 WHERE telegram_user_id = $2', [
