...
const LULU_API_URL =
  process.env.LULU_SANDBOX === 'true' ? 'https://api.sandbox.lulu.com' : 'https://api.lulu.com'
...
const PRINT_COLORS = [
  { id: 'BW', name: 'Black & White', description: 'Economical for text-heavy books' },
  {
    id: 'FC',
    name: 'Full Color',
    description: 'Vivid colors for photos and illustrations',
    popular: true
  }
]
...
const BINDING_TYPES = [
  {
    id: 'PB',
    name: 'Perfect Bound (Paperback)',
    description: 'Classic softcover with glued spine',
    popular: true
  },
  { id: 'CW', name: 'Case Wrap (Hardcover)', description: 'Premium hardcover with printed cover' },
  {
    id: 'DJ',
    name: 'Dust Jacket (Hardcover)',
    description: 'Hardcover with removable printed jacket'
  },
...
const PAPER_TYPES = [
  {
    id: '060UW',
    name: '60# Uncoated White',
    weight: 60,
    coated: false,
    color: 'white',
    description: 'Standard book paper',
    popular: true
  },
  {
    id: '070UW',
    name: '70# Uncoated White',
    weight: 70,
    coated: false,
    color: 'white',
    description: 'Slightly heavier'
  },
  {
    id: '080UW',
    name: '80# Uncoated White',
    weight: 80,
    coated: false,
    color: 'white',
    description: 'Premium weight'
  },
  {
    id: '050UC',
    name: '50# Uncoated Cream',
    weight: 50,
    coated: false,
    color: 'cream',
    description: 'Classic cream tone'
  },
  {
    id: '060UC',
    name: '60# Uncoated Cream',
    weight: 60,
    coated: false,
    color: 'cream',
    description: 'Warm classic feel',
    popular: true
  },
  {
    id: '070UC',
    name: '70# Uncoated Cream',
    weight: 70,
    coated: false,
    color: 'cream',
    description: 'Premium cream'
  },
  {
    id: '080CW',
    name: '80# Coated White',
    weight: 80,
    coated: true,
    color: 'white',
    description: 'Glossy photo paper'
  },
  {
    id: '100CW',
    name: '100# Coated White',
    weight: 100,
    coated: true,
    color: 'white',
    description: 'Heavy glossy paper'
  },
  {
    id: '070CW',
    name: '70# Coated White',
    weight: 70,
    coated: true,
    color: 'white',
    description: 'Standard glossy'
  },
  {
    id: '060CW',
    name: '60# Coated White',
    weight: 60,
    coated: true,
    color: 'white',
    description: 'Light glossy'
  }
]
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Record order ownership for IDOR prevention
    const db = req.app.locals.db
    await db.query(
      `INSERT INTO lulu_orders (user_id, lulu_order_id, status) VALUES ($1, $2, $3)
     ON CONFLICT (lulu_order_id) DO NOTHING`,
      [req.user.id, String(result.id), result.status?.name || 'created']
    )
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Verify ownership — return 404 to avoid leaking order existence
...
// Get list of orders (only user's own orders)
router.get(
  '/orders',
  requireDb,
  asyncHandler(async (req, res) => {
    ...
    // Get user's order IDs from our ownership table
    const userOrders = await db.query(
      'SELECT lulu_order_id FROM lulu_orders WHERE user_id = $1 ORDER BY created_at DESC',
      [req.user.id]
    )
    ...
    // Fetch each order from Lulu API
    const orders = await Promise.all(
      userOrders.rows.map(async row => {
        try {
          return await luluRequest(`/print-jobs/${row.lulu_order_id}/`)
        } catch {
          return { id: row.lulu_order_id, status: { name: 'unknown' } }
        }
      })
    )
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Update status in our tracking table
    await db.query("UPDATE lulu_orders SET status = 'cancelled' WHERE lulu_order_id = $1", [
      orderId
    ])
