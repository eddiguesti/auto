...
import { styleSchemas } from '../schemas/index.js'
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await db.query(
      'SELECT id, user_id, tones, narrative, author_style, applied_at FROM user_style_preferences WHERE user_id = $1',
      [userId]
    )
...
router.post(
  '/preferences',
  requireDb,
  validate(styleSchemas.savePreferences),
  asyncHandler(async (req, res) => {
    const db = req.app.locals.db
    const userId = req.user.id
    const { tones, narrative, authorStyle } = req.validatedBody
...
router.post(
  '/preview',
  requireDb,
  validate(styleSchemas.preview),
  asyncHandler(async (req, res) => {
    const db = req.app.locals.db
    const userId = req.user.id
    const { storyId, tones, narrative, authorStyle } = req.validatedBody
    ...
    const storyResult = await db.query(
      'SELECT id, answer, original_answer FROM stories WHERE id = $1 AND user_id = $2',
      [storyId, userId]
    )
...
router.post(
  '/apply-all',
  requireDb,
  validate(styleSchemas.applyAll),
  asyncHandler(async (req, res) => {
    const db = req.app.locals.db
    const userId = req.user.id
    const { tones, narrative, authorStyle } = req.validatedBody
    ...
    const storiesResult = await db.query(
      "SELECT id, answer, original_answer FROM stories WHERE user_id = $1 AND answer IS NOT NULL AND answer != ''",
      [userId]
    )
    ...
    const stylePrompt = buildStylePrompt(tones, narrative, authorStyle)
    const styleDescription = [...(tones || []), narrative, authorStyle].filter(Boolean).join(', ')
    ...
    for (const story of stories) {
      try {
        ...
        logger.error('Error transforming story', {
          storyId: story.id,
