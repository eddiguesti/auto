          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-lg border border-sepia/30 bg-white focus:outline-none focus:ring-2 focus:ring-sepia/50"
                onKeyDown={(e) => e.key === 'Enter' && saveName()}
              />
              <button
                onClick={saveName}
                className="px-6 py-2 bg-sepia text-white rounded-lg hover:bg-sepia/80 transition"
              >
                Start
              </button>
            </div>
      ...
      {!showNameInput && (
        <>
            ...
            {chapters.map((chapter) => {
              ...
              return (
                <Link
                  ...
                  className={`bg-gradient-to-br ${chapter.color} rounded-xl p-6 shadow-sm hover:shadow-md transition group`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <span className="text-3xl mb-2 block">{chapter.icon}</span>
                      <h2 className="text-xl font-bold text-ink group-hover:text-sepia transition">
                        {chapter.title}
                      </h2>
                      <p className="text-sepia/70 text-sm">{chapter.subtitle}</p>
                    ...
                    <div className="text-right">
                      ...
                      <div className="w-16 h-2 bg-white/50 rounded-full mt-1 overflow-hidden">
                        <div
          ...
          <div className="text-center">
            <Link
              to="/export"
              className="inline-flex items-center gap-2 px-6 py-3 bg-ink text-white rounded-lg hover:bg-ink/80 transition"
            >
